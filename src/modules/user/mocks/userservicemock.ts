...
when(mockedUserService.createUserIfFirstTimeLogin(userEmail)).thenReturn(
  new Promise((resolve, _) => {
    resolve(userDTO);
  }),
);

when(mockedUserService.findUserByEmailOrPhone(userEmail)).thenReturn(
  new Promise((resolve, _) => {
    resolve(Result.ok(user));
  }),
);

when(mockedUserService.getUser(userID)).thenReturn(
  new Promise((resolve, _) => {
    resolve(userDTO);
  }),
);
