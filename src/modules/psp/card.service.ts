...
export class CardService {
  ...
  public async addPaymentMethod(
    ...
    if (checkoutCustomerData.length === 0) {
      ...
      checkoutCustomerID = await this.checkoutService.createConsumer(consumer.props.email);
    } else {
      checkoutCustomerID = checkoutCustomerData[0].providerCustomerID;
    }

    const addPaymentMethodResponse: PspAddPaymentMethodResponse = await this.checkoutService.addPaymentMethod(
      paymentMethod,
      checkoutCustomerID,
    );
    ...
    let paymentResponse: PspPaymentResponse;
    ...
    if (validity == BINValidity.NOT_SUPPORTED) {
      ...
      try {
        ...
        paymentResponse = await this.checkoutService.makePayment(
          /* amount= */ 1,
          /* currency= */ "USD",
  ...
  async requestCheckoutPayment(consumer: Consumer, transaction: Transaction): Promise<PaymentRequestResponse> {
    const paymentResponse: PspPaymentResponse = await this.checkoutService.makePayment(
