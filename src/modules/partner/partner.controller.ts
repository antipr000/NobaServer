import { Body, Controller, ForbiddenException, Get, HttpStatus, Inject, Param, Post, Put, Req, Request } from '@nestjs/common';
...
export class PartnerController {
  ...
  constructor(
    ...
    @ApiResponse({status:HttpStatus.OK,   type: PartnerDTO, description: 'Returns the partner details of currently logged in partner admin'})
    ...
    async updateTakeRate(
      ...
      @Request() request): Promise<PartnerDTO> {
    ...
    @ApiResponse({ status: HttpStatus.OK, type: PartnerAdminDTO, description: 'Returns details for the requesting partner admin' })
    @ApiBadRequestResponse({ description: 'Not authorized' })
    async getPartnerAdmin(
        @Param(PartnerID) partnerID: string, 
        @Param(PartnerAdminID) partnerAdminID: string,
        @Request() request): Promise<PartnerAdminDTO> {
            const requestUser: PartnerAdmin = request.user;
            if(requestUser.props._id !== partnerAdminID && !requestUser.canGetAllAdmins())
