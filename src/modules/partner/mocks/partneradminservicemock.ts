...
when(
  mockedPartnerAdminService.addPartnerAdmin(
...
when(mockedPartnerAdminService.getPartnerAdmin(mockPartnerAdminWithAllAccess._id)).thenReturn(
  new Promise((resolve, _) => {
    resolve(partnerAdmin);
  }),
);

when(mockedPartnerAdminService.getPartnerAdmin(mockPartnerAdminWithBasicAccess._id)).thenReturn(
  new Promise((resolve, _) => {
    resolve(PartnerAdmin.createPartnerAdmin(mockPartnerAdminWithBasicAccess));
  }),
);

when(mockedPartnerAdminService.getPartnerAdmin(mockPartnerAdminWithIntermediateAccess._id)).thenReturn(
  new Promise((resolve, _) => {
    resolve(PartnerAdmin.createPartnerAdmin(mockPartnerAdminWithIntermediateAccess));
  }),
);

when(mockedPartnerAdminService.getAllPartnerAdmins(mockPartnerAdminWithAllAccess.partnerId)).thenReturn(
  new Promise((resolve, _) => {
    resolve([
      partnerAdmin,
...
when(mockedPartnerAdminService.getPartnerAdminFromEmail(mockFailureEmailAddress)).thenReject(
...
export { mockedPartnerAdminService };
