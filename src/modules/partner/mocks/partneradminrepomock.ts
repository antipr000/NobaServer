...
when(mockedPartnerAdminRepo.addPartnerAdmin(anything()))
    .thenReturn(new Promise((resolve, _) => {
        resolve(partnerAdmin);
    }));

when(mockedPartnerAdminRepo.getAllAdminsForPartner(partnerAdmin.props.partnerId))
    .thenReturn(new Promise((resolve, _) => {
        resolve([
            partnerAdmin,
...
when(mockedPartnerAdminRepo.getPartnerAdmin(partnerAdmin.props._id))
    .thenReturn(new Promise((resolve, _) => {
        resolve(Result.ok(partnerAdmin));
    }));

when(mockedPartnerAdminRepo.getPartnerAdminUsingEmail(partnerAdmin.props.email))
    .thenReturn(new Promise((resolve, _) => {
        resolve(Result.ok(partnerAdmin));
    }));

when(mockedPartnerAdminRepo.getPartnerAdminUsingEmail(mockFailureEmailAddress))
    .thenReturn(new Promise((resolve, _) => {
        resolve(Result.fail("Could not find admin with given email"))
    }));

when(mockedPartnerAdminRepo.removePartnerAdmin(partnerAdmin.props._id))
    .thenReturn(new Promise((resolve, _) => {
        resolve();
    }));

when(mockedPartnerAdminRepo.updatePartnerAdmin(deepEqual(PartnerAdmin.createPartnerAdmin({
    ...mockPartnerAdminWithAllAccess,
    name: mockPartnerAdminWithBasicAccess.name
})))).thenReturn(new Promise((resolve, _) => {
    resolve(PartnerAdmin.createPartnerAdmin({
        ...mockPartnerAdminWithAllAccess,
        name: mockPartnerAdminWithBasicAccess.name
    }));
}));
