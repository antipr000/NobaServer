...
describe('AdminService', () => {
  ...
  const testJwtSecret: string = "TEST_SECRET";
  ...
  describe('validateAndGetUserId', () => {
    it('should throw "NotFoundException" if user with given email doesn\'t exist', async () => {
      ...
      when(mockOtpRepo.getOTP(NON_EXISTING_ADMIN_EMAIL, identityType))
    ...
    it('should throw "UnauthorizedException" if otp is incorrect', async () => {
      ...
      when(mockOtpRepo.getOTP(EXISTING_ADMIN_EMAIL, identityType))
        .thenResolve(otpDomain);
