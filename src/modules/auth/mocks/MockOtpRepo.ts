...
export const getMockOtpRepoWithDefaults = () => {
  ...
  when(mockIOtpRepo.getOTP(anyString(), anyString(), anyString())).thenReject(new Error("Not implemented!"));
  when(mockIOtpRepo.getAllOTPsForUser(anyString(), anyString(), anyString())).thenReject(new Error("Not implemented"));
  when(mockIOtpRepo.saveOTP(anyString(), anyNumber(), anyString(), anyString(), anyString())).thenReject(
