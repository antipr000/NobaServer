...
export class EventHandler {
  ...
  public async sendOtp(payload: SendOtpEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.OTP_EMAIL[payload.locale], //this is template id for sending otp without any context, see sendgrid dashboard
      dynamicTemplateData: {
        user: payload.name ?? "",
  ...
  public async sendWalletUpdateVerificationCode(payload: SendWalletUpdateVerificationCodeEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.WALLET_UPDATE_OTP[payload.locale],
      dynamicTemplateData: {
        user: payload.name ?? "",
        user_email: payload.email,
        one_time_password: payload.otp,
  ...
  public async sendWelcomeMessage(payload: SendWelcomeMessageEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.WELCOME_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
  ...
  public async sendKycApprovedUSEmail(payload: SendKycApprovedUSEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.ID_VERIFICATION_SUCCESSFUL_US_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
      },
  ...
  public async sendKycApprovedNonUSEmail(payload: SendKycApprovedNonUSEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.ID_VERIFICATION_SUCCESSFUL_NON_US_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
      },
  ...
  public async sendKycDeniedEmail(payload: SendKycDeniedEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.KYC_DENIED_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
  ...
  public async sendKycPendingOrFlaggedEmail(payload: SendKycPendingOrFlaggedEvent) {
    ...
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.KYC_FLAGGED_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
  ...
  public async sendDocVerificationPendingEmail(payload: SendDocumentVerificationPendingEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.DOC_VERIFICATION_PENDING_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
      },
  ...
  public async sendDocVerificationRejectedEmail(payload: SendDocumentVerificationRejectedEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.DOC_VERIFICATION_REJECTED_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
      },
  ...
  public async sendDocVerificationFailedTechEmail(payload: SendDocumentVerificationTechnicalFailureEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.DOC_VERIFICATION_FAILED_TECH_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
      },
  ...
  public async sendCardAddedEmail(payload: SendCardAddedEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.CARD_ADDED_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
  ...
  public async sendCardAdditionFailedEmail(payload: SendCardAdditionFailedEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.CARD_ADDITION_FAILED_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
        last_four: payload.last4Digits,
  ...
  public async sendCardDeletedEmail(payload: SendCardDeletedEvent) {
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.CARD_DELETED_EMAIL[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
        card_network: payload.cardNetwork,
  ...
  public async sendTransactionInitiatedEmail(payload: SendTransactionInitiatedEvent) {
    ...
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.TRANSACTION_INITIATED_EMAIL[payload.locale],
      dynamicTemplateData: {
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
  ...
  public async sendCryptoFailedEmail(payload: SendCryptoFailedEvent) {
    ...
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.CRYPTO_FAILED_EMAIL[payload.locale],
      dynamicTemplateData: {
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
        transaction_id: payload.params.transactionID,
        user_email: payload.email,
  ...
  public async sendOrderExecutedEmail(payload: SendOrderExecutedEvent) {
    ...
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.ORDER_EXECUTED_EMAIL[payload.locale],
      dynamicTemplateData: {
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
        transaction_id: payload.params.transactionID,
        user_email: payload.email,
  ...
  public async sendOrderFailedEmail(payload: SendOrderFailedEvent) {
    ...
    const msg = {
      to: payload.email,
      from: SENDER_EMAIL,
      templateId: EmailTemplates.ORDER_FAILED_EMAIL[payload.locale],
      dynamicTemplateData: {
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
        transaction_id: payload.params.transactionID,
        user_id: payload.email,
  ...
  public async sendHardDeclineEmail(payload: SendHardDeclineEvent) {
    const msg = {
      ...
      from: SENDER_EMAIL,
      templateId: EmailTemplates.NOBA_INTERNAL_HARD_DECLINE[payload.locale],
      dynamicTemplateData: {
        user_email: payload.email,
        username: Utils.getUsernameFromNameParts(payload.firstName, payload.lastName),
