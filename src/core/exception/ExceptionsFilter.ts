...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  constructor(private logger:Logger){};
    ...
    if(true){
      ...
      const serverError = isApplicationException(originalException)? (originalException as any as ApplicationException).getDetailsForServer(): {};

      this.logger.error( messageToBeLogged, { serverError: serverError, timestamp} );
    }
    ...
    response.status(status).json({
      statusCode: status,
      details: isApplicationException(originalException) || Joi.isError(originalException)? httpException.getResponse() : null,
      message: message,
      timestamp: timestamp, 
      path: request.url,
    });
