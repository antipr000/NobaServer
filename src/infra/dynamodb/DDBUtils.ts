...
export type CrudOptions = {
  // any one of them should be present
  isCreating?: boolean;
  isReading?: boolean;
  isUpdating?: boolean;
  isDeleting?: boolean;
...
export function addGSIAttributes<T>(props: T, gsiConsumedByModel: GSIConsumer<T>[]): T {
  for (const gsiConsumer of gsiConsumedByModel) {
    ...
    if (gsiConsumer.gsiPK && gsiConsumer.constGSIPK) {
      throw new Error(
        "GSI Consumer cannot define both const partition key and partitionKeyAttribute in model to be used!!",
      );
    }
...
export function toDDBModelInstance<T extends DynamoDBModel>(
  raw: any,
  ...
  if (options.isCreating) {
    ...
    throw new Error(
      "crud operation to be performed on entity is not defined i.e. we need to know if this is a first time creation of entity or update, data:" +
        JSON.stringify(raw),
    );
  }
...
export async function getNoneOrOneElseThrow<T>(iterator: QueryIterator<T>): Promise<T> {
  ...
  if (results.length > 1) {
    throw new DuplicateItemException({
      message: `duplicate items found, items: ${JSON.stringify(results)}`,
    });
