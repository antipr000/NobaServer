...
import { DeleteOptions, DynamoDbTable, getSchema, PutOptions, StringToAnyObjectMap, UpdateOptions, DataMapper, DataMapperConfiguration } from '@aws/dynamodb-data-mapper';
import {
    isKey,
     ...
     */
     updateRequest<T extends StringToAnyObjectMap = StringToAnyObjectMap>(
        ...
        let {
            condition,
            onMissing = 'remove',
            skipVersionCheck = this._skipVersionCheck,
        } = options;
        const schema = getSchema(item);
